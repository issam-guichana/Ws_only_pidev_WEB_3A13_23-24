 ...
 */
public function show(int $idEvent): Response
{
    $entityManager = $this->getDoctrine()->getManager();

    // Retrieve the Evenement entity by its ID
    $evenement = $entityManager->getRepository(Evenement::class)->find($idEvent);

    // If the Evenement entity is not found, throw a 404 Not Found exception
    if (!$evenement) {
        throw $this->createNotFoundException('Evenement not found');
    }

    // Retrieve related UsrEvt entities for the given Evenement
    $usrEvts = $entityManager->getRepository('App\Entity\UsrEvt')->findBy(['event' => $evenement]);
    ...
    /**
     * @Route("/edit/{idEvent}", name="app_evenement_edit", methods={"GET", "POST"})
     */
    public function edit(Request $request, int $idEvent, EntityManagerInterface $entityManager): Response
    {
        // Retrieve the Evenement entity by its ID
        $evenement = $entityManager->getRepository(Evenement::class)->find($idEvent);
        ...
        // Create the form using the EvenementType and pass the Evenement entity
        $form = $this->createForm(EvenementType::class, $evenement);
        $form->handleRequest($request);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($imageFile) {
                ...
                try {
                    ...
                    $this->addFlash('error', 'Failed to upload the image.');
                    return $this->redirectToRoute('app_evenement_edit', ['idEvent' => $evenement->getIdEvent()]);
                }
            }
